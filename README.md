...
주문·배송에 대해 자주 받는 질문과 답을 모은 페이지입니다.

### 답할 수 있는 질문만 질문으로 만듭니다

배송 소요일·결제수단·교환반품 조건은 아직 정해지지 않았습니다. 그것들을
질문으로 늘어놓고 전부 "확정 예정" 이라고 답하면 사용자는 답을 못 얻고,
답변엔진은 **"AVORA 는 이 질문에 답하지 않는다"** 를 배웁니다.

그래서 미확정 항목은 개별 질문으로 만들지 않고 아래쪽 한 블록으로 묶어,
이미 목록을 갖고 있는 배송·교환반품 페이지로 보냅니다. 같은 목록을 두 곳에서
관리하지 않기 위해서입니다.
...
| 답 | 원본 |
|---|---|
| 배송비 | `src/config/commerce.json` — 배송 안내 페이지와 **같은 문구 키**를 씁니다 |
| 판매 국가 | `src/config/payment-config.json` |
| 회원가입 필요 여부 | `ACCOUNTS_ENABLED` |
| 연락처 | `src/config/site.ts` 의 `BUSINESS` |

문장에 값을 적어 넣지 않으므로 정책이 바뀌면 이 페이지가 따라오고, 결제 화면과
어긋날 수가 없습니다. `tests/e2e/support.spec.ts` 가 설정과 화면을 대조합니다.

### 구조화 데이터에는 확정된 답만

배송비는 아직 잠정값이라 화면에는 단서와 함께 내보내되 `FAQPage` 스키마에서는
뺍니다(`schema: false`). "아직 정해지지 않았습니다" 를 구조화 데이터로 내보내는
것은 답변엔진에 그 문장을 먹이는 일입니다.

질문마다 `id` 를 달아 `#shipping-fee` 처럼 특정 답을 가리켜 인용할 수 있습니다.
제품 페이지 FAQ 와 질문이 겹치지 않는 것도 테스트가 확인합니다 — 같은 질문이
두 페이지에 있으면 어느 쪽이 정본인지 알 수 없습니다.

### 모바일에서 찾아가는 길

헤더 링크(`.nav__links`)는 900px 미만에서 숨겨집니다. 그 폭에서는 헤더의
**메뉴 시트**(`src/components/MenuSheet.astro`)가 같은 링크를 담습니다.
JS 가 없으면 시트는 열리지 않으므로, 같은 링크를 푸터에 평문으로 항상 둡니다.

## 모바일 메뉴

900px 미만에서 헤더 링크가 숨겨지므로, 그 폭에서는 메뉴 버튼(☰)이 바텀시트를
엽니다. 브랜드 스토리 · 제품 · 고객센터 · (회원 기능을 켜면) 마이페이지.

### 왜 언어 시트와 같은 형태인가

같은 헤더에서 열리는 것이 둘인데 형태가 다르면 사용자가 같은 화면에서 상호작용을
두 번 배워야 합니다. `LanguageSwitcher` 의 구조를 그대로 따릅니다 — `<dialog>` +
`showModal()`, 배경 클릭 닫기, Esc, 닫으면 열었던 버튼으로 포커스 복귀.

`<dialog>` 를 쓰는 이유도 같습니다. `.nav` 에 `backdrop-filter` 가 걸려 있어
`position: fixed` 자손의 기준이 뷰포트가 아니라 nav 가 됩니다. `showModal()` 은
top layer 에 그려져 그 영향을 받지 않습니다.

### 마이페이지가 헤더가 아니라 시트에 있는 이유

커머스를 켜면 헤더 우측이 장바구니 · 마이페이지 · 언어 · 메뉴 넷이 됩니다.
360px 에서 워드마크(120px)와 좌우 여백(40px)을 빼면 200px 밖에 남지 않아
들어가지 않습니다. 장바구니는 배지가 붙는 거래 단축키라 헤더에 남기고,
마이페이지가 한 단계 들어갑니다.

법적 페이지(약관·개인정보·배송)는 메뉴에 넣지 않습니다. 메뉴는 짧을수록
쓸모 있고, 그것들의 정본 자리는 푸터입니다.

### 검사

`tests/e2e/mobile-menu.spec.ts` 가 **닫는 방법 셋**(닫기 버튼·Esc·배경 클릭),
포커스 복귀, 현재 페이지 표시, 5개 언어, 320~430px 넘침, 탭 영역 44px,
900px 이상에서 숨는지를 봅니다. 모달은 여는 것보다 **닫는 것**이 어렵습니다.

`tests/e2e/a11y.spec.ts` 는 **시트가 열린 상태**를 axe 로 검사합니다 —
그 폭에서만 나오는 요소라 데스크톱 크기로만 검사하면 아무도 보지 않습니다.
...
그 선언만 조용히 무효가 되고, 속성마다 다르게 무너집니다.

| 속성 | 무효일 때 | 증상 |
|---|---|---|
| `color` | 상속값으로 떨어짐 | 브랜드 잉크 대신 순수 검정 |
| `background` | 초기값(투명) | 요소가 아예 안 보임 |
| `border-color` | `currentColor` | 비슷해 보여 못 알아챔 |
| `box-shadow` | 선언 전체 무효 | 그림자가 사라짐 |

실제로 `--brand-ink` 가 **21곳에서 쓰이는데 어디에도 정의돼 있지 않았습니다**
(토큰 생성기가 만드는 이름은 `--color-ink` 입니다). 그 결과:

- 사이트 전체 본문이 Deep Forest 가 아니라 **순수 검정**이었습니다
- 브랜드 강조 밑줄(`.mark`)이 밝은 섹션에서 **보이지 않는 것을 애니메이션**하고
  있었습니다 — 어두운 섹션은 다른 토큰을 써서 정상이라 더 눈에 띄지 않았습니다

기존 검사가 못 잡은 이유가 중요합니다. `tokens-contrast.spec.ts` 는 팔레트 값
**끼리** 대조할 뿐 토큰이 화면에 닿는지는 보지 않고, axe 대비 검사는 검정이
Deep Forest 보다 **대비가 더 높아** 통과시킵니다.

`tests/e2e/tokens-defined.spec.ts` 가 두 가지를 봅니다.

1. 스타일시트와 `admin.astro` 의 모든 `var(--…)` 이름이 `tokens.css` 에 있는지
2. 실제 렌더링 — 본문 글자색이 `rgb(35,41,31)` 인지, 강조 밑줄이 투명이 아닌지
