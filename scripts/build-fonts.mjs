...
export function postStringsFor(locale, postsRoot = POSTS_ROOT) {
  ...
  for (const file of readdirSync(dir)) {
     ...
     * 초안은 건너뜁니다.
     *
     * `draft: true` 인 글은 **페이지가 만들어지지 않습니다**(`src/lib/posts.ts`).
     * 그 글자는 화면 어디에도 나오지 않는데, 여기서 함께 세면 서브셋에 실려
     * 모든 한국어 페이지가 아무도 볼 수 없는 글자를 받게 됩니다. 저널 초안
     * 두 편을 넣었을 때 `ko/body.woff2` 가 5KB 늘었습니다.
     *
     * 이 판정은 **과소 집계가 아닙니다.** 초안이 공개되는 순간 이 조건이
     * 풀리고, `--check` 가 같은 함수를 쓰므로 그때 "서브셋에 없는 글자" 로
     * 걸립니다. 즉 빠뜨린 채로 배포될 수 없습니다.
     *
     * ⚠️ **프론트매터 안에서만 찾습니다.** 처음에는 원문 전체에 정규식을
     * 걸었는데, 그러면 본문에 `draft: true` 라는 줄이 있는 글(프론트매터를
     * 코드 블록으로 보여주는 글이 그렇습니다)이 통째로 건너뛰어집니다. 그
     * 글의 글자가 서브셋에서 빠지고, `--check` 도 같은 함수를 지나므로
     * **아무도 잡지 못한 채** 화면에서 서체가 섞입니다.
     *
     * 값을 파싱하지 않고 줄로 찾는 것은 그대로입니다 — 파서가 한 줄짜리
...
export function charsFor(locale, postsRoot = POSTS_ROOT) {
   ...
   * 통째로 없었습니다. 언어 이름은 `한국어`·`Tiếng Việt` 이 같은 상태였습니다.
   *
   * 빌드는 통과하고 화면도 "글자는 보이므로" 멀쩡해 보입니다. macOS 는
   * 시스템 서체가 받아 주지만 Windows·Android 에서는 자간과 굵기가 눈에
   * 띄게 어긋납니다 — 만든 사람 화면에서는 드러나지 않는 종류입니다.
   *
   * ⚠️ `简体中文`·`ไทย` 는 이걸로 해결되지 않습니다. body 서체(Pretendard)에
   * 애초에 그 글자가 없어서, 요구 목록에 넣어도 서브셋에 들어갈 수 없습니다
   * (검사도 `source.has(ch)` 로 걸러 이 경우를 폴백이 정상이라고 봅니다).
   * 언어 전환기의 중국어·태국어 이름은 지금도 시스템 서체가 그립니다 —
   * 그건 이 목록이 아니라 서체 선택의 문제입니다.
   *
