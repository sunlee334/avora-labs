...
const NAV: readonly NavGroup[] = [
  ...
  },
  {
     ...
     * 검증단.
     *
     * `/panel` 은 배점표까지 갖춘 채로 만들어져 있었는데 **헤더에도 푸터에도
     * 링크가 없었습니다.** 홈 중간 섹션의 CTA 와 FAQ 한 줄이 전부였고, 둘 다
     * 홈을 끝까지 내려야 나옵니다. 다른 화면(제품·브랜드·고객센터)에서 들어온
     * 사람에게는 검증단이 존재하지 않는 것과 같았습니다.
     *
     * 모집은 2026년 10월입니다. 착지점을 먼저 만들어 두고 진입로를 내지
     * 않은 셈이라, 크루 담당자에게 링크를 뿌리기 전에 닫아야 하는 구멍입니다.
     *
     * ── 왜 5개 언어에 전부 내보내는가 ─────────────────────────
     * 모집 대상은 국내 러닝 크루지만 `/panel` 은 이미 다섯 벌이 다 있고
     * 번역돼 있습니다(그 페이지 머리주석 참고 — 한국어판만 색인합니다).
     * 언어마다 메뉴 항목 수가 달라지면 "정의는 한 곳" 이라는 이 파일의
     * 성질이 언어 조건으로 갈라지고, 세 화면을 대조하는 테스트도 언어별
     * 예외를 알아야 합니다. **색인은 페이지가 정하고, 메뉴는 존재만 봅니다.**
     */
    id: 'panel',
    label: (t) => t.nav.panel,
