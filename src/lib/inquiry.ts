 ...
 *
 * ── 실패 응답만의 이야기가 아닙니다 ────────────────────────
 * 본문을 안 쓰는 곳이면 성공이든 실패든 같습니다. 문의 제출은 201 과 함께
 * 만들어진 문의를 돌려주는데(`worker/index.ts:655`) 화면은 그것을 쓰지 않고
 * 목록을 다시 불러옵니다 — 그래서 **더 흔한 성공 경로가 더 오래 샜습니다.**
 *
 * ── 왜 `body?.cancel()` 이 아닌가 ──────────────────────────
 * 여기 본문은 수십 바이트짜리 JSON 입니다. 끝까지 읽으면 스트림이 정상
 * 종료되어 커넥션이 재사용됩니다. `cancel()` 은 중단을 신호하고, 본문이 없는
 * 응답에서는 `res.body` 가 `null` 이라 `?.` 가 조용히 아무것도 안 합니다 —
 * 헬퍼의 계약이 응답 모양에 의존하게 됩니다. `cancel()` 이 이기는 것은
 * 본문이 클 때뿐입니다.
 */
export async function drain(res: Response): Promise<void> {
  try {
    ...
    // 던지는 경우는 둘뿐입니다 — 본문이 이미 소비됐거나(여기선 불가능),
    // 받는 중 연결이 끊겼거나. 둘 다 여기서 할 수 있는 조치가 없습니다.
  }
}
...
export function mountInquiry(
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      // 201 본문에 만들어진 문의가 들어 있지만 쓰지 않습니다 — 바로 아래에서
      // 목록을 다시 불러오기 때문입니다. 안 쓸 본문도 닫아야 합니다.
      await drain(res);
