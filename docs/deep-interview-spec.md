...
## 2차 — 자사 결제 개발 완료 (2026-08-25)

### Round 6 결정의 번복과 그 이유
Round 6 에서 "자체 DB 없음 — 주문·결제 기록은 PG 대시보드와 이메일 영수증"으로 정했으나,
체크아웃을 설계하며 구멍이 드러났습니다. **PG 대시보드에는 결제 정보만 남고, 수령인·연락처·
배송지는 우리 폼이 받는 정보라 어딘가 남기지 않으면 "결제는 됐는데 어디로 보낼지 모르는 주문"**이
됩니다. 사용자 확인을 거쳐 **Cloudflare D1 에 주문을 저장**하는 것으로 변경했습니다.

부수 효과로 **금액 조작 방어**가 가능해졌습니다. 주문을 먼저 서버에 만들어 금액을 기억해 두고,
승인 직전에 대조합니다. 브라우저가 보낸 금액을 그대로 믿으면 값을 바꿔 싸게 결제할 수 있습니다.

### 만들어진 것
| 영역 | 내용 |
|---|---|
| 장바구니 | localStorage. 담기·수량·삭제·헤더 배지·다른 탭 동기화·손상값 복구 |
| 체크아웃 | 배송지 폼(모바일 키보드 타입·44px·인라인 검증), 결제수단 선택, 주문 요약, 개인정보 동의 |
| 주문 API | `POST /api/orders` 주문 생성(금액 확정) · `POST /api/payments/confirm` 승인 · `POST /api/orders/lookup` 비회원 조회 |
| 주문 완료 | 승인 호출 + 결과 표시. 새로고침해도 중복 승인되지 않음 |
| 주문 조회 | 주문번호 + 연락처 둘 다 맞아야 조회. 응답에 연락처·메모 미포함 |
| D1 | `migrations/0001_orders.sql` |
| 개인정보 | 수집 항목 표를 코드 기준으로 작성. 고지 문안은 비움 |

### 보안 설계와 검증
| 위협 | 대응 | 검증 |
|---|---|---|
| 결제 금액 조작 | 주문 생성 시 항목 합계 대조, 승인 시 서버 저장 금액 대조 | E2E `AMOUNT_MISMATCH` · `AMOUNT_TAMPERED` |
| 조작 후 재시도 | 조작이 감지되면 주문을 `failed` 로 못박음 | E2E 상태 확인 |
| 완료 페이지 새로고침 중복 결제 | 이미 `paid` 면 재승인하지 않고 그대로 반환 | E2E `alreadyPaid` |
| 주문번호 추측으로 남의 배송지 열람 | 조회에 연락처 일치 요구, 불일치와 미존재를 같은 404 로 응답 | E2E |
| 주문 정보 캐싱 | 모든 주문 응답에 `Cache-Control: no-store` | E2E |
| 결제수단 정보 유출 | 카드번호 등을 아예 받지 않음 (PG 화면 안에서만 처리) | 설계 |

### 두 모드 테스트 체계
사이트가 설정에 따라 두 모드로 동작하는데 한 모드만 테스트하면 나머지는 아무도 확인하지 않은 채
배포됩니다. `E2E_MODE` 로 빌드와 대상 테스트를 함께 바꾸고 CI 가 둘 다 돌립니다.
- `commerce` 156개 — 장바구니·체크아웃·주문 API
- `launch` 98개 — 외부몰 연결 상태, PG 미설정 503

승인 경로는 실제 PG 없이는 도달할 수 없어 **테스트 전용 mock 어댑터**(`worker/payments/mock.ts`)를
두었습니다. `PAYMENT_PROVIDER=mock` 일 때만 잡히며 운영 설정에는 넣지 않습니다.

### 측정 (모바일 Lighthouse, 2차 코드 포함)
| 페이지 | Perf | A11y | LCP | CLS |
|---|---|---|---|---|
| /ko/ | 99 | 100 | 2.3s | 0 |
| /ko/product | 97 | 100 | 2.5s | 0 |
| /ko/cart | 98 | 100 | 2.3s | 0 |
| /ko/checkout | 99 | 100 | 2.2s | 0 |
| /ko/legal/privacy | 99 | 100 | 2.2s | 0 |
| /ko/order/lookup | 99 | 100 | 2.1s | 0 |

개발 중 checkout CLS 가 0.092 로 기준(0.1)에 근접해, 폼이 펼쳐질 때 푸터가 밀리는 것이 원인임을
확인하고 컨테이너에 최소 높이를 주어 0 으로 낮췄습니다. 회귀 테스트를 함께 넣었습니다.

### 결제를 켤 때 필요한 것 (순서대로)
1. PG 계약 → 2. `wrangler d1 create avora-orders` 후 `database_id` 기입 → 3. `npm run db:migrate`
→ 4. `wrangler secret put TOSS_SECRET_KEY` → 5. `vars.PAYMENT_PROVIDER` 설정
→ 6. `product.json` 가격 기입 → 7. `payment-config.json` 의 `KR.checkout` 을 `internal` 로
→ 8. `commerce.json` 배송비 정책 확정 → 9. `checkout.astro` 의 PG SDK 호출 연결

### 남은 것
- **PG SDK 호출 미연결** — PG 미정이라 결제창을 띄우는 자리에 주석으로 표시만 해 두었습니다.
  현재는 주문 생성 후 완료 페이지로 넘어가 승인 흐름까지 이어서 확인할 수 있습니다.
- **배송비 정책 미확정** — `commerce.json` 이 "전국 무료"로 되어 있으나 확정된 정책이 아닙니다.
- **개인정보 고지 문안 미작성** — 수집 항목 표만 코드 기준으로 채웠습니다.
- **주문 관리 화면 없음** — 판매자가 주문을 보려면 지금은 `wrangler d1 execute` 로 조회해야 합니다.
