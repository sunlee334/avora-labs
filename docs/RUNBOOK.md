...
## 1-b. Workers Free 플랜에서 운영할 때 (2026-09-11 결정: 당분간 Free 유지)

- 한도: 요청 **하루 10만 건**(정적 자산 제외), 요청당 **CPU 10ms**, D1 읽기 하루 500만 행·쓰기 10만 행, Time Travel 복구 **7일**.
- 현재 요청당 평균 CPU 는 약 23ms 이고 로그인(scrypt 해시, 약 20ms)도 정상 동작합니다. Cloudflare 가 이 구간을 바로 끊지는 않지만, 한도를 넘는 요청이 종료되면 로그에 `outcome: exceededCpu` 또는 브라우저에 **Error 1102** 가 나타납니다. 이게 보이기 시작하면 Paid(월 5달러)로 올리는 것이 유일한 해결책입니다 — 코드 변경 없이 결제만 하면 됩니다.
- 하루 요청이 8만 건을 넘기 시작하면(펀딩·광고 유입) 미리 올리세요. 한도를 넘으면 그날 남은 시간 동안 사이트가 열리지 않습니다.
- D1 복구 창이 7일이므로 GitHub 일일 내보내기(`docs/D1_BACKUP.md`)가 실질적인 백업입니다. 저장소 시크릿이 등록돼 있는지 확인하세요.
