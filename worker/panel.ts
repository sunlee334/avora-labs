...
export type ApplyResult = 'ok' | 'updated';
 ...
 * 사고로 이어집니다.
 *
 * 처음 동의한 시각(`consented_at`)은 덮어씁니다 — 다시 낼 때도 동의는 다시
 * 받으므로 마지막 동의 시각이 유효한 값입니다.
 */
  ...
  const updated = await db
    .prepare(
      `UPDATE panel_applications
          SET name = ?, activity = ?, frequency = ?, region = ?, locale = ?,
              consented_at = ?, marketing_at = ?
        WHERE email = ?`,
    )
    .bind(a.name, a.activity, a.frequency, a.region, a.locale, iso, marketingAt, a.email)
    .run();
  if ((updated.meta?.changes ?? 0) > 0) return 'updated';

  await db
    .prepare(
      `INSERT INTO panel_applications
         (id, name, email, activity, frequency, region, locale,
          created_at, consented_at, marketing_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(
