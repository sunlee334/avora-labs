...
test.describe('푸터 모양', () => {
  for (const width of [900, 1024, 1280, 1600]) {
    test(`${width}px — 묶음 넷이 한 줄에 선다`, async ({ page }) => {
       ...
       *
       * ── 한계선을 800 에서 900 으로 옮겼습니다 ────────────────
       * 800 은 푸터가 685px 이던 시절의 여유였습니다. 원칙은 그 숫자가 아니라
       * 위에 적힌 **화면 한 장(900px)** 입니다.
       *
       * 푸터 바닥에 브랜드 워드마크가 들어오면서(clamp 72~280px) 1280px 에서
       * 879px 이 됩니다. 크기를 줄여 800 을 지키려면 115px 까지 내려야 하는데,
       * 그건 원래 있던 76px 과 거의 같아 "브랜드명이 한 번은 크게 나온다" 가
       * 성립하지 않습니다.
       *
       * 그래서 여유가 아니라 **원칙의 실제 경계** 로 잽니다. 900 을 넘으면
       * 푸터만 보이는 화면이 생기고, 그건 여전히 결함입니다.
       *
       * 기준값이 685 에서 709 로 오른 것은 푸터 메뉴가 여섯에서 여덟로
       * 늘었기 때문입니다 — 배치 문제가 아니라 항목이 는 것입니다.
       */
      const coarse = await page.evaluate(() => matchMedia('(pointer: coarse)').matches);
      expect(height, `푸터가 ${Math.round(height)}px 입니다`).toBeLessThan(coarse ? 980 : 900);
    });
  }
