...
test.describe('데이터베이스가 유령 행을 막는다', () => {
  ...
  /**
   * 이것이 잡는 것은 **과도한 제약** 입니다 — CHECK 가 정당한 행까지 막는
   * 경우입니다. 위 테스트는 그것을 볼 수 없습니다.
   *
   * (처음에는 "테이블이 없어도 위 테스트가 통과하니까" 를 근거로 적었는데
   * 틀렸습니다. 위 단언은 `.toThrow()` 가 아니라 `.toThrow(/CHECK constraint
   * failed/)` 이고, 테이블이 없으면 `no such table: inquiries` 가 나므로
   * 정규식에 걸리지 않아 이미 빨갛게 실패합니다. 배포된 커밋 메시지에도 같은
   * 틀린 근거가 남아 있어 여기에 정정을 남깁니다.)
   */
